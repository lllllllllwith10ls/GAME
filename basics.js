	...
	constructor(x,y,vx,vy) {
		...
		this.shoot = function() {
			if(this.cooldown >= this.reload) {
				let angle = this.angle+Math.PI+Math.random()-0.5*Math.PI/20; 
...
const updateGame = () => {
	...
	if (mouse.down === true) {
		...
		if(player.cooldown >= player.reload) {
			let angle = Math.atan2(player.pos.x-mouse.pos.x,player.pos.y-mouse.pos.y)+Math.random()-0.5*Math.PI/20; 
